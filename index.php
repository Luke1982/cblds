...
if ((!$viewAttachment) && (!$viewAttachment && $action!='home_rss') && $action!=$Ajx_module.'Ajax' && $action!='massdelete' && $action!='DashboardAjax' && $action!='ActivityAjax') {
	if ((!$skipFooters) && $action != "ChangePassword" && $action != "body" && $action != $Ajx_module."Ajax" && $action!='Popup' && $action != 'ImportStep3' && $action != 'ActivityAjax' && $action != 'getListOfRecords') {
		...
		$smarty->assign('coreBOS_uiapp_name', $coreBOS_uiapp_name);
		$smarty->assign('coreBOS_uiapp_companyname', $coreBOS_uiapp_companyname);
		$smarty->assign('coreBOS_uiapp_version', $coreBOS_uiapp_version);
		$smarty->assign('coreBOS_uiapp_url', $coreBOS_uiapp_url);
		$smarty->assign('coreBOS_uiapp_showgitversion', $coreBOS_uiapp_showgitversion);
		$smarty->assign('coreBOS_uiapp_showgitdate', $coreBOS_uiapp_showgitdate);
		$smarty->assign('gitversion', $gitversion);
		$smarty->assign('gitdate', $gitdate);
		$smarty->assign('deltaTime', $deltaTime);
		$smarty->assign('calculate_response_time', $calculate_response_time);
